...
class GuessPadViewControllerSnapshotTests: XCTestCase {
    ...
    func test_emptyInputWithFourDigit() {
        ...
        record(snapshot: sut.snapshot(for: .iPhone8(style: .light)), named: "EMPTY_INPUT_PAD_FOUR_DIGIT_light")
        record(snapshot: sut.snapshot(for: .iPhone8(style: .dark)), named: "EMPTY_INPUT_PAD_FOUR_DIGIT_dark")
    }
    
    func test_emptyInputWithFiveDigit() {
        let sut = makeSUT(digitCount: 5)
        
        record(snapshot: sut.snapshot(for: .iPhone8(style: .light)), named: "EMPTY_INPUT_PAD_FIVE_DIGIT_light")
        record(snapshot: sut.snapshot(for: .iPhone8(style: .dark)), named: "EMPTY_INPUT_PAD_FIVE_DIGIT_dark")
    }
